...
public sealed partial class DamageVisualsSystem : VisualizerSystem<DamageVisualsComponent>
{
    ...
    private void UpdateDamageVisuals(List<ProtoId<DamageGroupPrototype>> delta, Entity<DamageableComponent, SpriteComponent, DamageVisualsComponent> entity)
    {
        ...
        {
            if (!damageVisComp.Overlay && damageGroup.Id != damageVisComp.DamageGroup)
                continue;
            ...
            if (!damageVisComp.LastThresholdPerGroup.TryGetValue(damageGroup.Id, out var lastThreshold)
                || !CheckThresholdBoundary(damageTotal, lastThreshold, damageVisComp, out var threshold))
                continue;

            damageVisComp.LastThresholdPerGroup[damageGroup.Id] = threshold;

            if (damageVisComp.TargetLayers != null)
            {
                foreach (var layerMapKey in damageVisComp.TargetLayerMapKeys)
                {
                    UpdateTargetLayer((entity, spriteComponent, damageVisComp), layerMapKey, damageGroup.Id, threshold);
                }
            }
            else
            {
                UpdateOverlay((entity, spriteComponent, damageVisComp), damageGroup.Id, threshold);
            }
        }
    ...
    private void ForceUpdateLayers(Entity<DamageableComponent, SpriteComponent, DamageVisualsComponent> entity)
    {
        ...
        if (damageVisComp.DamageOverlayGroups != null)
        {
            UpdateDamageVisuals(damageVisComp.DamageOverlayGroups.Keys.Select(group => new ProtoId<DamageGroupPrototype>(group)).ToList(), entity);
        }
        else if (damageVisComp.DamageGroup != null)
        {
            UpdateDamageVisuals(new List<ProtoId<DamageGroupPrototype>> { damageVisComp.DamageGroup }, entity);
        }
        else if (damageVisComp.DamageOverlay != null)
        {
