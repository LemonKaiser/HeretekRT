...
/// <summary>
/// Reusable rounded client surface with independently tinted fill and border.
/// It draws the contour directly because the stock Nano SVG is an opaque white fill,
/// so layering it would destroy alpha values of dark UI surfaces.
/// </summary>
public sealed class HeretekRoundedStyleBox : StyleBox
{
    ...
    // Style boxes are drawn serially by the UI. Reusing these buffers avoids allocations every frame.
    ...
    protected override void DoDraw(DrawingHandleScreen handle, UIBox2 box, float uiScale)
    {
        ...
        if (thickness.Left <= 0f && thickness.Top <= 0f && thickness.Right <= 0f && thickness.Bottom <= 0f)
            return;
        ...
        FillVertices(innerBox, MathF.Max(0f, radius - MathF.Max(thickness.Left, thickness.Top)), _innerContour);
    ...
    private static void FillVertices(UIBox2 box, float radius, Vector2[] vertices)
    {
        ...
        for (var index = 0; index < UnitContour.Length; index++)
        {
            ...
            vertices[index] = center + UnitContour[index] * radius;
        }
    }
