    ...
    public sealed class StorageTest
    {
        ...
        public async Task StorageSizeArbitrageTest()
        {
            ...
            await server.WaitAssertion(() =>
            {
                ...
                {
                    if (!proto.TryComp<StorageComponent>(compFact.CompName<StorageComponent>(), out var storage) ||
                        storage.Whitelist != null ||
                        storage.MaxItemSize == null ||
                        !proto.TryComp<ItemComponent>(compFact.CompName<ItemComponent>(), out var item))
                        continue;
        ...
        public async Task TestStorageFillPrototypes()
        {
            ...
            await server.WaitAssertion(() =>
            {
                Assert.Multiple(() =>
                {
                    foreach (var proto in protoManager.EnumeratePrototypes<EntityPrototype>())
                    {
                        if (!proto.TryComp<StorageFillComponent>(compFact.CompName<StorageFillComponent>(), out var storage))
                            continue;
        ...
        public async Task TestSufficientSpaceForFill()
        {
            ...
            await Assert.MultipleAsync(async () =>
            {
                foreach (var (proto, fill) in pair.GetPrototypesWithComponent<StorageFillComponent>())
                {
                    ...
                    await server.WaitAssertion(() =>
                    {
                        if (!proto.TryComp<StorageComponent>(compFact.CompName<StorageComponent>(), out storage))
                        {
                            Assert.Fail($"Entity {proto.ID} has storage-fill without a storage component!");
                            return;
                        }

                        proto.TryComp<ItemComponent>(compFact.CompName<ItemComponent>(), out item);
                    ...
                    foreach (var entry in fill.Contents)
                    {
                        ...
                        {
                            fillItem.TryComp<ItemComponent>(compFact.CompName<ItemComponent>(), out entryItem);
                        });
        ...
        public async Task TestSufficientSpaceForEntityStorageFill()
        {
            ...
            foreach (var (proto, fill) in pair.GetPrototypesWithComponent<StorageFillComponent>())
            {
                ...
                await server.WaitAssertion(() =>
                {
                    if (!proto.TryComp<EntityStorageComponent>(compFact.CompName<EntityStorageComponent>(), out var entStorage))
                        Assert.Fail($"Entity {proto.ID} has storage-fill without a storage component!");
        ...
        private int GetEntrySize(EntitySpawnEntry entry, bool getCount, IPrototypeManager protoMan, SharedItemSystem itemSystem, IComponentFactory compFact)
        {
            ...
            if (proto.TryComp<ItemComponent>(compFact.CompName<ItemComponent>(), out var item))
            {
                if (proto.TryComp<StackComponent>(compFact.CompName<StackComponent>(), out var stack))
                {
