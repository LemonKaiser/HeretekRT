...
public sealed partial class BuildMech : IGraphAction
{
    ...
    public void PerformAction(EntityUid uid, EntityUid? userUid, IEntityManager entityManager)
    {
        ...
        {
            Logger.GetSawmill("construction").Warning($"Mech construct entity {uid} did not have a container manager! Aborting build mech action.");
            return;
        }
        ...
        {
            Logger.GetSawmill("construction").Warning($"Mech construct entity {uid} did not have the specified '{Container}' container! Aborting build mech action.");
            return;
        }

        if (container.ContainedEntities.Count != 1)
        {
            Logger.GetSawmill("construction").Warning($"Mech construct entity {uid} did not have exactly one item in the specified '{Container}' container! Aborting build mech action.");
        }
        ...
        {
            Logger.GetSawmill("construction").Warning($"Mech construct entity {uid} had an invalid entity in container \"{Container}\"! Aborting build mech action.");
            return;
        }
