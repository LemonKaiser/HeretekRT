...
public sealed partial class JobCondition : EntityEffectCondition
{
    ...
    public override bool Condition(EntityEffectBaseArgs args)
    {
        ...
        foreach (var roleId in mind.MindRoles)
        {
            ...
            {
                Logger.GetSawmill("entity_effects").Error($"Encountered job mind role entity {roleId} without a {nameof(MindRoleComponent)}");
                continue;
            }

            if (mindRole.JobPrototype == null)
            {
                Logger.GetSawmill("entity_effects").Error($"Encountered job mind role entity {roleId} without a {nameof(JobPrototype)}");
                continue;
            }
