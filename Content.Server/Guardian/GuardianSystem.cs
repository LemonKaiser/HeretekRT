    ...
    public sealed partial class GuardianSystem : EntitySystem
    {
        ...
        private void OnDoAfter(EntityUid uid, GuardianCreatorComponent component, DoAfterEvent args)
        {
            ...
            if (TryComp<GuardianComponent>(guardian, out var guardianComp))
            {
                ...
                _audio.PlayPvs(new Robust.Shared.Audio.SoundPathSpecifier("/Audio/Effects/guardian_inject.ogg"), args.Args.Target.Value);
        ...
        private void OnHostStateChange(EntityUid uid, GuardianHostComponent component, MobStateChangedEvent args)
        {
            ...
            if (args.NewMobState == MobState.Critical)
            {
                ...
                _audio.PlayPvs(new Robust.Shared.Audio.SoundPathSpecifier("/Audio/Effects/guardian_warn.ogg"), component.HostedGuardian.Value);
            }
            else if (args.NewMobState == MobState.Dead)
            {
                //TODO: Replace WithVariation with datafield
                _audio.PlayPvs(new Robust.Shared.Audio.SoundPathSpecifier("/Audio/Voice/Human/malescream_guardian.ogg"), uid, AudioParams.Default.WithVariation(0.20f));
