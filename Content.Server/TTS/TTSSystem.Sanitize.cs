...
public sealed partial class TTSSystem
{
    internal static string Sanitize(string text)
    {
        ...
        text = UnsupportedSymbols().Replace(text, "");
        text = text.Replace("+", "");
        text = WordPattern().Replace(text, ReplaceMatchedWord);
        text = LatinLetters().Replace(text, ReplaceLat2Cyr);
        text = DecimalSeparator().Replace(text, " целых ");
    ...
    [GeneratedRegex(@"\[[^\]]*\]")]
    private static partial Regex TagStripper();

    [GeneratedRegex(@"[^a-zA-Zа-яА-ЯёЁ0-9,\-+?!. ']+")]
    private static partial Regex UnsupportedSymbols();

    [GeneratedRegex(@"(?<![a-zA-Zа-яА-ЯёЁ])[a-zA-Zа-яА-ЯёЁ]+\d*(?![a-zA-Zа-яА-ЯёЁ])", RegexOptions.IgnoreCase)]
    ...
    [GeneratedRegex(@"(?<=\d)[.,](?=\d)")]
    private static partial Regex DecimalSeparator();
