...
public abstract partial class SharedChameleonClothingSystem : EntitySystem
{
    ...
    protected void UpdateVisuals(EntityUid uid, ChameleonClothingComponent component)
    {
        ...
        if (TryComp(uid, out ClothingComponent? clothing) &&
            proto.TryComp(Factory.CompName<ClothingComponent>(), out ClothingComponent? otherClothing))
        {
        ...
        if (TryComp(uid, out AppearanceComponent? appearance) &&
            proto.TryComp(Factory.CompName<AppearanceComponent>(), out AppearanceComponent? appearanceOther))
        {
        ...
        if (proto.TryComp(Factory.CompName<ContrabandComponent>(), out ContrabandComponent? contra))
        {
    ...
    public bool IsValidTarget(EntityPrototype proto, SlotFlags chameleonSlot = SlotFlags.NONE, string? requiredTag = null)
    {
        ...
        if (!proto.TryComp(out TagComponent? tag, Factory) || !_tag.HasTag(tag, "WhitelistChameleon"))
            return false;
        ...
        if (!proto.TryComp(Factory.CompName<ClothingComponent>(), out ClothingComponent? clothing))
            return false;
