...
public static class ScreenCheckImageValidator
{
    ...
    public const byte BlackPixelComponentThreshold = 8;
    ...
    /// <summary>
    /// Detects an image that is effectively black without treating the result as invalid data.
    /// This is diagnostic information for renderer failures, not an anti-cheat signal.
    /// </summary>
    public static bool IsLikelyBlackRgb(ReadOnlySpan<byte> rgbData)
    {
        const int channelsPerPixel = 3;
        const int maxSamples = 4096;

        var pixelCount = rgbData.Length / channelsPerPixel;
        if (pixelCount == 0)
            return false;

        var sampleStep = Math.Max(1, pixelCount / maxSamples);
        for (var pixelIndex = 0; pixelIndex < pixelCount; pixelIndex += sampleStep)
        {
            var offset = pixelIndex * channelsPerPixel;
            if (rgbData[offset] > BlackPixelComponentThreshold
                || rgbData[offset + 1] > BlackPixelComponentThreshold
                || rgbData[offset + 2] > BlackPixelComponentThreshold)
            {
                return false;
            }
        }
